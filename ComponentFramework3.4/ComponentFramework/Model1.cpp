...
Model1::Model1() :quadSphere(nullptr), skybox(nullptr), shader(nullptr), skyboxshader(nullptr) {
	OnCreate();
}
...
#undef TEXCOORD_LENGTH
	...
	modelMatrix = MMath::lookAt(Vec3(0.0, 0.0, 8.0), Vec3(0.0, 0.0, 0.0), Vec3(0.0, 1.0, 0.0));
...
void Model1::Render(const Matrix4& projectionMatrix, const Matrix4& modelViewMatrix, const Matrix3& normalMatrix) const {

	glUseProgram(shader->getProgram());
	glBindBuffer(GL_ARRAY_BUFFER, vbo);
	glUniformMatrix4fv(projectionMatrixID, 1, GL_FALSE, projectionMatrix);
	glUniformMatrix4fv(modelViewMatrixID, 1, GL_FALSE, modelViewMatrix * modelMatrix);
